...
Because this is a large project and I had **limited** time, I treated it as a **Minimum Viable Product (MVP)**. I focused on the following **crucial** elements:

- That the project is **functional**.
- That the design is as **similar** as the Figma reference.
- That the code structure is **functional** and minimally **organized**.

### Things I Left Out Due to Time Constraints:

- Tests
- Animations

## Development Shortcuts Taken

- **Responsiveness:** The site is responsive, but development primarily targeted **1920x1080** and **iPhone SE** screen sizes as representative samples. With more time, I would have addressed a wider range of screen sizes.
- **Refactoring:** While I refactored parts of the application, further global refactoring was considered **overkill** given the project's scope.
- **External Library Use:** I leveraged an external library for **toast** notifications.
- **CSS Class Management:** I intended to use a utility function, like **`cn`** (classnames), for managing Tailwind CSS classes but skipped this step due to time constraints.
- **State Handling:** I significantly minimized time spent implementing robust loading and error states.
- **Design Polish:** I was unable to polish the design to the desired level of detail.
- **CSS Variables:** I did not use global variables for the CSS colors.

## External Libraries

I used these tools because they **standardize processes**, **optimize** the app, and are common **industry standards**:

- **`react-hook-form`** and **`zod`** for form validation.
- **`swr`** for the frontend requests.
- **`react-hot-toast`** for the toasts.
- Prisma, the ORM for the database.
